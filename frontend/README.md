...
Leave it running. Sanity check (any terminal): `curl localhost:8000/` →
`{"status":"ok",...}`.
...
**4. Click through in the browser** — open http://localhost:5173:
   ...
   5. **Log out** (header) → you're routed back to login.
...
```bash
VITE_API_BASE=http://localhost:8000   # separate-host dev (default)
...
├── hooks/useTrips.ts     # GET /trips with refetchInterval polling
...
- **Feedback capture** on a completed trip — T1 Stage 0, see
  `docs/agent-state-and-memory-roadmap.md`.
